...
- ~~**Propaga para os filhos** por padrão: um componente no root do painel deixa o painel inteiro cinza.~~ **Não propaga** *(decidido em 15/09/2026)*: um componente por imagem. O uGUI só aplica material e mesh modifier por componente no próprio GameObject do Graphic, então propagar exigiria adicionar componentes ocultos nos filhos ou trocar o material deles — os dois mexem em objetos que o usuário não tocou. Para aplicar em várias imagens, multi-seleção + Add Component.
- **Decisões de implementação** *(15/09/2026)*: intensidade em **UV0.z** e intensidade do tint em **UV0.w** (o Canvas sempre envia UV0; confirmar que vai com 4 componentes); cor do tint no material, com um material compartilhado por cor; shader chega ao build por **referência padrão serializada** no componente (sem Resources, sem material para arrastar — verificado: a referência fica no `.meta` do script, e os 24 componentes da cena de sandbox salvaram `m_Shader` apontando para o shader; na 6.6 até o `AddComponent` feito no Editor a aplica, então só `AddComponent` em player fica sem ela e cai no `Shader.Find`); Graphic com material customizado é **pulado com aviso** no inspector; precisão `half` na luminância.
...
- **Inspector mostra quem controla quem** *(pedido do autor em 16/09/2026, após teste manual em URP)*: as duas regras que atravessam GameObjects (`Selectable` acima e `ImplicitGrayscale` ancestral) eram invisíveis fora da documentação. No filho, um info box nomeia o botão e o ancestral que o deixam cinza; no pai, lista as imagens abaixo que seguem o seu `Selectable` e as que ele mantém pelo menos tão cinzas (só as diretas, sem pular um ancestral mais próximo), com botão para selecionar. Calculado pelos mesmos métodos que o runtime usa (`FindSelectable`/`FindAncestor`), então não diverge. Limite aceito: um `Button` sem `ImplicitGrayscale` não mostra a lista, porque não desenhamos no inspector de componentes da Unity; o aviso aparece só nos filhos. Só com um objeto selecionado. Agrupado por objeto *(ajuste de 16/09/2026)*: a primeira versão repetia o mesmo filho em duas caixas quando o botão e o grayscale pai estavam no mesmo objeto (caso mais comum); agora é uma caixa por objeto controlador no filho, e uma caixa só no pai, com uma linha e o motivo por filho e um botão Select ao lado.
...
2. **`RectMask2D` e softness.** A softness e as propriedades `_UIMaskSoftnessX/Y` foram adicionadas depois do lançamento inicial do `RectMask2D`, e a TMP levou ainda mais tempo para respeitá-las. Se o shader de grayscale copiar o `UI-Default` da versão certa, isso se resolve sozinho — mas **confirmar em qual das versões da matriz o `UI-Default` já tem a softness** antes de assumir.
   *(Verificado em 15/09/2026: o `UI-Default.shader` do arquivo oficial de built-in shaders da **2021.3.45f2** — o piso da matriz — já tem `_UIMaskSoftnessX/Y`, a máscara no interpolador `TEXCOORD2`, os dois `multi_compile_local`, instancing e stereo. O `UnityUI.cginc` é idêntico na 2021.3.45f2 e na 6000.6. Licença do arquivo: MIT, "Copyright (c) 2016 Unity Technologies". Atenção: o `package.json` declara `"unity": "2021.3"`, que aceita patches anteriores à 45 — não verificados.)*
...
**Hosts criados em 15/09/2026**, fora do repositório, em `MyProjects\ImplicitUI\PipelineHosts\`: `ImplicitUI-BuiltIn` (template `com.unity.template.3d`) e `ImplicitUI-HDRP` (template `com.unity.template.hdrp-blank`, HDRP 17.7.0), ambos na 6000.6.0f1, com o package referenciado por caminho (`file:../../../ImplicitUI/Packages/com.bionics.implicitui`) — mudanças no package aparecem neles sem copiar nada. A cena `Assets/ImplicitUIPipelineTest.unity` de cada host tem os mesmos casos de grayscale em um Canvas Screen Space - Overlay, um Screen Space - Camera e um World Space; ela é gerada por `AgentScripts/BuildPipelineHostScene.cs`, copiado para o `Assets/Editor` do host e rodado em batch mode (instruções no próprio arquivo). O URP é testado no projeto de desenvolvimento. **Resultado do teste manual — 16/09/2026:** URP e Built-in sem defeitos nos três modos. HDRP: o grayscale funciona nos três modos, sem variante de shader, então a decisão (a/b/c) abaixo não precisou ser acionada. Duas observações do HDRP, as duas reproduzidas **sem** o componente e portanto fora do nosso escopo: (1) Camera e World ficam mais escuros que o Overlay, inclusive a cor de referência e os textos — o `DefaultSettingsVolumeProfile` do HDRP usa **Tonemapping ACES**, aplicado a canvases desenhados antes do pós-processamento; o Overlay vem depois. Para comparar as colunas, `AgentScripts/HdrpUiReferenceVolume.cs` (copiado para o `Assets/Editor` do host) adiciona um Volume global sem tonemapping; (2) um frame preto em Camera/World ao entrar em Play, causa não identificada (não é exposição, que não afeta shaders de UI). **Builds de player (16/09/2026):** Windows (Mono, stripping High) e **Android (IL2CPP, ARM64, stripping High)** com a cena de sandbox do grayscale, verificados no aparelho pelo autor — todos os casos corretos. O shader entrou no build pela referência padrão e compilou para GLES3 e Vulkan sem erro. Build de aparelho por `AgentScripts/BuildDeviceSandbox.cs`, sempre numa cópia do projeto (o build regrava settings do URP e do Player). iOS/Metal não testado (exige Mac).
...
4. ~~**HDRP**: decidir entre variante própria, Overlay-only ou desistir~~ — **resolvido em 16/09/2026**: o grayscale funciona em HDRP nos três modos sem variante (§11.4).
